...
#         "timestamp":    datetime.fromtimestamp(datetime.now().timestamp() - 288 * (2000 - i)).isoformat()
#     }
...
for i in range(0, 2100):
    if 0<=i<300: #day1
        if 0<=i<150: #100 happies
            json_object = {
                "sentiment":    "happiness",
                "timestamp":    datetime.fromtimestamp(datetime.now().timestamp() - 288 * (2100 - i)).isoformat()
            }
            data_generation(json_object)
            print(i)
